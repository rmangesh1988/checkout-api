...
public class CheckoutService {
  ...
  public CheckoutResponse performCheckout(List<CheckoutItem> checkoutItems) {
    ...
    var productIds =
        checkoutItems.stream().map(CheckoutItem::productId).collect(Collectors.toSet());
    var productsInCatalogue = productCatalogueService.findByProductIdIn(productIds);
    if (productIds.size() != productsInCatalogue.size()) {
      var productIdsInCatalogue =
          productsInCatalogue.stream().map(Product::getProductId).collect(Collectors.toSet());
      var inValidProductIds =
          new HashSet<>(CollectionUtils.removeAll(productIds, productIdsInCatalogue));
      throw new EntityNotFoundException(
          String.format(PRODUCT_NOT_FOUND, String.join(", ", inValidProductIds)));
    }

    // Aggregate selected product ids
    var selectedProductIdsCount =
    ...
    var totalPrice =
        selectedProductIdsCount.entrySet().stream()
            .mapToDouble(e -> calculateIndividualProductPrice(e, productsInCatalogue))
            .sum();
    return new CheckoutResponse(totalPrice);
  }

  private double calculateIndividualProductPrice(
      Map.Entry<String, Integer> entry, List<Product> products) {
    var product =
        products.stream()
            .filter(p -> entry.getKey().equalsIgnoreCase(p.getProductId()))
            .findFirst()
            .get();
    ...
    if (discountQuantity != 0) {
      ...
      finalPriceOfProduct =
          (quantityWithoutDiscount * unitPrice)
              + ((totalQuantity - quantityWithoutDiscount)
                  / discountQuantity
