...
@ActiveProfiles("test")
@SpringBootTest(classes = CheckoutApiApplication.class, webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
...
class CheckoutApiIntegrationTest {
    ...
    void shouldReturnTotalCostWhenGivenAListOfProducts() {
        ...
        var checkoutResponseResponseEntity = testRestTemplate.exchange(createURLWithPort(V1_CHECKOUT_URI), HttpMethod.POST, httpEntity, CheckoutResponse.class);

        assertEquals(HttpStatusCode.valueOf(OK.value()), checkoutResponseResponseEntity.getStatusCode());
    ...
    void shouldThrowExceptionAsNotFoundWhenInvalidProductIdsAreProvided() {
        ...
        var httpEntity = new HttpEntity<>(checkoutItems);
        var apiExceptionResponseResponseEntity = testRestTemplate.exchange(createURLWithPort(V1_CHECKOUT_URI), HttpMethod.POST, httpEntity, ApiExceptionResponse.class);

        assertEquals(HttpStatusCode.valueOf(NOT_FOUND.value()), apiExceptionResponseResponseEntity.getStatusCode());
        assertEquals(APPLICATION_JSON, apiExceptionResponseResponseEntity.getHeaders().getContentType());
        assertEquals(String.format(PRODUCT_NOT_FOUND, "00111"), apiExceptionResponseResponseEntity.getBody().getMessage());
    }
    ...
    void shouldThrowExceptionAsBadRequestWhenBlankProductIdsAreProvided() {
        ...
        assertEquals(HttpStatusCode.valueOf(BAD_REQUEST.value()), apiExceptionResponseResponseEntity.getStatusCode());
        assertEquals(APPLICATION_JSON, apiExceptionResponseResponseEntity.getHeaders().getContentType());
